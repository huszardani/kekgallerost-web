...
test("a kezdeti kísérlet és három retry után végleges failed állapot keletkezik", async () => {
  ...
  for (let attempt = 1; attempt <= MAX_APPLICATION_EMAIL_ATTEMPTS; attempt += 1) {
    ...
    assert.equal(result.status, attempt < 4 ? "pending" : "failed");
  }
...
test("az adminnézet mindhárom címzetti szerep állapotát és próbálkozását képezi", () => {
  const rows = applicationEmailAdminStatuses([
    { id: "a", template_key: "application_confirmation", recipient_role: "applicant", status: "sent", attempt_count: 1, last_attempt_at: "2026-08-11T10:00:00Z", next_attempt_at: null, error_code: null, error_message: null },
    { id: "p", template_key: "application_notification_partner", recipient_role: "partner", status: "queued", attempt_count: 2, last_attempt_at: "2026-08-11T10:00:00Z", next_attempt_at: "2026-08-11T10:30:00Z", error_code: "provider_temporarily_unavailable", error_message: "Átmeneti hiba." },
    { id: "m", template_key: "application_notification_admin", recipient_role: "admin", status: "failed", attempt_count: 4, last_attempt_at: "2026-08-11T10:00:00Z", next_attempt_at: null, error_code: "recipient_missing_or_invalid", error_message: "provider raw detail must stay hidden" },
  ]);
  assert.deepEqual(rows.map((row) => [row.role, row.status, row.email?.attempt_count]), [
    ["applicant", "elküldve", 1], ["partner", "függőben", 2], ["admin", "sikertelen", 4],
  ]);
